...
from src.data_modules.lusifer_datasets import LusiferDataset, ConcatLusiferDataset, LusiferCollator
...
class LusiferDataModule(L.LightningDataModule):
    def __init__(
            self, 
            langs: List[str],
    ...
    def connect(
            ...
            world_size: int = 1,
            global_rank: int = 0,
            input_tokenizer: PreTrainedTokenizer = None, 
            output_tokenizer: PreTrainedTokenizer = None,
            ) -> None:
    ...
    def setup(self, stage: str = None) -> None:
        ...
        self.train_dataset = ConcatLusiferDataset(train_datasets) 
...
if __name__=='__main__':
    ...
    input_tokenizer = create_tokenizer('FacebookAI/xlm-roberta-large', 'xlm-r')
    output_tokenizer = create_tokenizer("nvidia/NV-Embed-v2", 'nvidia/NV-Embed-v2')
    dm = LusiferDataModule(
        langs=['en', 'zh'],
    ...
    dm.connect(world_size=1, global_rank=0, input_tokenizer=input_tokenizer, output_tokenizer=output_tokenizer)
    dm.prepare_data()
    dm.setup()
